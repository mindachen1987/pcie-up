...
static u32 pcie_events(struct plda_pcie *port)
{
...
static u32 sec_errors(struct plda_pcie *port)
{
	void __iomem *ctrl_base_addr = port->bridge_addr + MC_PCIE_CTRL_ADDR_OFFSET;
...
static u32 ded_errors(struct plda_pcie *port)
{
	void __iomem *ctrl_base_addr = port->bridge_addr + MC_PCIE_CTRL_ADDR_OFFSET;
...
static u32 local_events(struct plda_pcie *port)
{
...
static int mc_pcie_init_irq_domains(struct plda_pcie *port)
{
	struct device *dev = port->dev;
	...
	ret = plda_init_intx_irq_domain(port, dev, pcie_intc_node);
	if (ret) {
		of_node_put(pcie_intc_node);
		return ret;
	}
...
static int mc_init_interrupts(struct platform_device *pdev, struct mc_pcie *port)
{
	...
	struct plda_pcie *plda = &port->plda;
	int irq;
	int i, intx_irq, msi_irq, event_irq;
	int ret;

	ret = mc_pcie_init_irq_domains(plda);
	if (ret) {
	...
	for (i = 0; i < NUM_EVENTS; i++) {
		event_irq = irq_create_mapping(plda->event_domain, i);
		if (!event_irq) {
	...
	intx_irq = irq_create_mapping(plda->event_domain,
	...
	irq_set_chained_handler_and_data(intx_irq, mc_handle_intx, plda);

	msi_irq = irq_create_mapping(plda->event_domain,
				     EVENT_LOCAL_PM_MSI_INT_MSI);
	if (!msi_irq)
		return -ENXIO;

	/* Plug the MSI chained handler */
	irq_set_chained_handler_and_data(msi_irq, mc_handle_msi, plda);

	/* Plug the main event chained handler */
	irq_set_chained_handler_and_data(irq, mc_handle_event, plda);
...
static int mc_platform_init(struct pci_config_window *cfg)
{
	...
	plda_pcie_enable_msi(&port->plda);

	/* Configure non-config space outbound ranges */
	ret = plda_pcie_setup_iomems(&port->plda, bridge);
	if (ret)
		return ret;
